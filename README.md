...
- V1 建议以免费 / 买断授权音效为主，避免直接使用网易云等平台版权内容。
- 冥想内容支持两条路径：
  - 直接使用本地冥想音频
  - 通过服务端调用百度或讯飞 TTS，生成并缓存到本地离线播放
...
1. 替换真实白噪音 / 自然音资源并补充版权说明。
2. 接入 TTS 生成流程，建议将云函数密钥放在服务端。
3. 增加 Apple HealthKit 与华为 Health Kit 原生桥接。
4. 接入 Bugly / 听云等国内监控服务。
5. 增加订阅、会员权益、离线缓存、AB 实验与灰度发布。
